...
test('ready-to-ship fails with PAYMENT_REQUIRED when paidAt is missing', async () => {
  (prisma.order.findFirst as any) = async () => ({
    id: 'order-5',
    status: 'PAID',
    paidAt: null,
...
test('ready-to-ship passes for paid order with pvz data (post mock-success state)', async () => {
  (prisma.order.findFirst as any) = async () => ({
    id: 'order-6',
    status: 'PAID',
    paidAt: new Date(),
