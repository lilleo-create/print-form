...
const createThreadSchema = z.discriminatedUnion("kind", [
  z.object({
    ...
    try {
      ...
      if (payload.kind === "SELLER") {
        const seller = await prisma.user.findFirst({
          where: { id: payload.sellerId, role: "SELLER" },
        ...
        if (existing) {
          return res.json({ data: existing, meta: { created: false } });
        }
        ...
        const thread = await prisma.chatThread.create({
          data: {
            ...
            userId,
            sellerId: payload.sellerId,
            sellerShopName: seller.sellerProfile?.storeName ?? seller.name,
          },
        });

        return res.status(201).json({ data: thread, meta: { created: true } });
      }
